export default {
 title: "芸術家に取ってポータル",
 Nav: {
   signIn: "サインイン",
   signUp: "サインアップ",
   signOut: "サインアウト",
   account: "アカウント",
   home: "ホーム",
   groups: "グループ",
   friends: "友達",
   search: "検索",
   profile: "プロフィール"
  },
 NavForm: {
   setErrorMessageCreate: "登録することが出来ません。",
   theSameEmail: "このメールのアカウントはいます。代わりにサインインして下さい。",
   unVerified: "メールがベリファイいませんでした。",
   wrongLoginData: "メールパスワードの組み合わせが正しくありません。",
   statusLogin: "ログイン中している。",
   notExist: "ユーザーは生きませｎ。",
   validateRequired: "所要",
   validateUsernameFl: "第一字が大きいでなければなりません。",
   validateUsernameHKik: "名前は字だけ受け入れます。今、ひらがなとカタカナと漢字が出来ます。",
   validateUsernameNum: "名前がナンバーを含めることが出来ません。",
   validateUsernameMin: "名前が短すぎまあす。",
   validatePseudonymNum: "あだ名が少なくとも一ナンバーがでなければなりません。",
   validatePseudonymSpec: "あだ名が少なくとも一特殊文字を含めることが出来ます: #?!@$%^&*-＃？！＄％＆＊ー",
   validatePseudonymHKik: "あだ名は字だけ受け入れます。今、ひらがなとカタカナと漢字が出来ます。",
   validatePseudonymMin: "あだ名が短すぎまあす。",
   validatePseudonymMax: "あだ名が長すぎる。最大限十五字をなければなりません。",
   validateEmail: "メールが正しくないです。",
   validatePasswordNum: "パスポートが短すぎまあす。 最大限九字をなければなりません。",
   validatePasswordOl: "パスポート少なくとも一大きいでなければなりません。",
   validatePasswordHKik: "パスポートは字だけ受け入れます。今、ひらがなとカタカナと漢字が出来ます。",
   validatePasswordOn: "パスポートが少なくとも一ナンバーがでなければなりません。",
   validatePasswordSpec: "パスポートが少なくとも一特殊文字を含めることが出来ます: #?!@$%^&*-",
   createSubmit: "登録してます",
   loginSubmit: "ログイン",
   titleOfRegistration: "Pfartistsへようこそ！",
   titleOfLogin: "ログイン！",
   email: "メール",
   password: "パスポート",
   loadingRegistration: "登録してます。。。",
   successInfoRegistration: "おめでとうございます！登録するのはしてました。 メールの受信箱を確認し、メールを確認してログインします。",
   forgottenPasswordLink: "パスポートが忘れました",
   providerTitleRegistration: "それともで、使って登録します：",
   providerTitleLogin: "それともで、経由でログインします：",
   acceptInfoOne: "登録することにより、お客様は当社の",
   acceptInfoTwo: "利用規約",
   acceptInfoThree: "および",
   acceptInfoFour: "プライバシーポリシー",
   dot: "に同意したものとみなされます。",
   changeToCreate: "すでにアカウントをお持ちの方？",
   changeToLogin: "アカウントをお持ちでない方？"
  },
 Aside: {
   category: "部分け",
   drawings: "描画",
   realistic: "写実的",
   manga: "マンガ",
   anime: "アニメ",
   comics: "コミック",
   photographs: "写真",
   animations: "アニメーション",
   videos: "映画",
   others: "他",
   friends: "フェイバリットの友達",
   groups: "フェイバリットの団体",
   addingGroup: "団体が増設",
   photos: "フォト"
  },
 Footer: {
   termsOfUse: "利用条件",
   privacyPolice: "プライバシーポリシー",
   contact: "連絡",
   plans: "プラン",
   faq: "FAQ",
   changeLanguage: "言語"
  },
 App: {
   lastDrawings: "最近の描画",
   lastPhotos: "最近の写真",
   lastOthers: "最近の他",
   lastAnimations: "最近のアニメーション",
   lastVideos: "最近の映画"
  },
 Account: {
   aMenu: {
     general: "一般",
     gallery: "ギャラリー",
     profile: "プロフィール",
     friends: "友達",
     groups: "私の団体"
    },
   aData: {
     subscription: "サブスクリプション",
     currentPlan: "現在の計画",
     changeButton: " 変わります",
     actualEmail: "現在のメール",
     changeEmail: "メールが変わります",
     currentPassword: "現在のパスポート入ります",
     oldPassword: "古いがパスポートが入ります",
     newPassword: "新しいパスポートが入ります",
     againNewPassword: "新しいパスポートが繰り返します",
     changePassword: "パスポートが 変わります",
     Premium: {
       header: "サブスクリプションが変わります",
       select__error: "プランが選びませんでした。",
       body: "計画を知る様に、この ",
       bodyLink: "リンク",
       bodyDot: "がクリックします。",
       update: "アップデート",
      }
    },
   gallery: {
     userPhotosTitle: "貴職の写真と素描",
     userLikedPhotos: "好きの写真と素描",
     userVideosTitle: "貴職の映画",
     userLikedVideos: "好きの映画",
     userAnimationsTitle: "貴職のアニメーション",
     userLikedAnimations: "好きのアニメーション",
    },
   profile: {
     aboutMe: "貴職は",
     ariaLabelButton: "Updating pseudonym and description button",
     save: "セーブ",
     successSending: "Profile has been updated.",
     errorSending: "Profile hasn't been updated.",
     defaultAvatar: "default avatar",
     userAvatar: "avatar",
    },
   groups: {
     adminTitle: "生み出しました団体",
     noAdmin: "まだ団体が生み出しませんでした。",
     modsTitle: "取り締ました団体",
     noMods: "まだモデレータがいません。",
     usersTitle: "付けました団体",
     noUsers: "まだ団体まで付けませんでした。",
    }
  },
 Main: {
   title: "はじめまして、芸術家！",
   firstQuestion: "Are you searching a service, which will be dedicated you?",
   secondQuestion: "Are oyu searching a service will hold up as graphical diary? And maybe do you want to show off your artistic works?",
   firstAnswer: "You hit good! This is service dedicated such people as you.",
   secondAnswer: "From Sunday artists to people creating their virtual handy portfolios.",
   containerFirstQuestion: "Short film? Any Gif? Drawing? Picture? Photo?",
   containerFirstAnswer: "No problem! You send and already done!",
   containerSecondQuestion: "Do you want to see, what others are doing?",
   containerSecondAnswer: "Click on the nickname and browse.",
   containerThirdQuestion: "Do you want to see what is at the top?",
   containerThirdAnswer: "You already have this on the main page. Being enough you log in!",
   containerFourthQuestion: "Do you want to see what lately you liked?",
   containerFourthAnswer: "Being enough you log in and you already have this!",
   containerFifthQuestion: "Design?",
   containerFifthAnswer: "Of course minimalist! Yours to be on top, not ours. This also makes everything clearer.",
   containerSixthQuestion: "Don't you like bright site?",
   containerSixthAnswer: "So click and you've Dark mode!",
   containerSeventhQuestion: "Do you want to find something about specific type?",
   containerSeventhAnswer: "Choose the appropriate category.",
   containerEighthQuestion: "Do you search people about similar likings?",
   containerEighthAnswer: "Choose a group or, better, you'll create it yourself!",
   containerNinthQuestion: "Do you have artistic friends here?",
   containerNinthAnswer: "You always have their on hand. Fast, so you know what they showed off recently."
  },
 NewUser: {
   title: "Adding your name and pseudonym",
   name: "名前",
   ariaLabelButton: "sending first data.",
   successSending: "Profile has been updated.",
   errorSending: "Profile hasn't been updated."
  },
 AnotherForm: {
   pseudonym: "あだ名",
   profilePhoto: "プロフィールの写真",
   description: "叙述",
   fileTitle: "ファイルが追加",
   tags: "タグ",
   file: "ファイル",
   send: "送る",
   uploadFile: "File has been uploaded.",
   notUploadFile: "File hasn't been uploaded."
  },
 AddingGroup: {
   title: "Create a group",
   name: "Group name"
  },
 Groups: {
   join: "付ける",
   joined: "付けました",
   favorite: {
     addedToFav: "愛用",
     addToFavorite: "愛用まで付け加える",
     maxFav: "五団体まで付け加える出来る。",
     maximumAchieved: "もう五愛用の団体います。"
    },
   menu: {
     members: "隊員"
    },
   addingPost: {
     add: "追加が追加する",
     addTitPlaceholder: "名称",
     addTitAria: "新しい追加に名称立項",
     addDescription: "叙述",
     addDesAria: "新しい追加に叙述立項"
    },
   list: {
     title: "リストの団体",
     more: "もっと",
     all: "これがもう皆あります。"
    },
   noGroups: "愛用の団体がありません",
   noPermission: "追加が見られる様に、この団体が付けないと行けない。"
  },
 Friends: {
   add: "友達まで付け加える",
   added: "友達",
   addFav: "愛用まで付け加える",
   addedFav: "愛用",
   max: "五団体まで付け加える出来る。",
   addedMax: "もう五愛用の団体います。",
   noFriends: "友達がいません",
   noFavFriends: "愛用がいません"
  },
 groupsUser: {
   adminTitle: "ユーザが生み出しました団体",
   noAdmin: "ユーザがまだ団体が生み出しませんでした。",
   modsTitle: "ユーザが取り締ました団体",
   noMods: "ユーザがまだモデレータがいません。",
   usersTitle: "ユーザが付けました団体",
   noUsers: "ユーザがまだ団体まで付けませんでした。"
  },
 Posts: {
   likeAria: "好き",
   likedAria: "好きだ",
   noPosts: "追加がありません"
  },
 Members: {
   admin: "アドミン",
   moderators: "モデレータ",
   modsAria: "モデレータがリムーブ",
   noMods: "モデレータがいません",
   anotherMembers: "他人の隊員",
   noMembers: "隊員がいません",
   addModAria: "モデレータが付け加える"
  },
 Description: {
   textPlaceholder: "新しい叙述の団体",
   textAria: "新しい叙述の団体の場",
   submit: "アップデート",
   iconButton: "叙述のメニューのアドミン"
  },
 Comments: {
   comments: "コメント",
   reply: "答えます",
   newComPlaceholder: "新しいコメントが書きます",
   newComAria: "新しいファイルに叙述立項",
   newComButton: "コメントが付け加えます",
   noComments: "コメントがありません",
   updateTitle: "コメントがエディト",
   updateButton: "アップデート",
   deleteCommentTitle: "コメントが消す"
  },
 Regulations: {
   regulation: "レギュレーション",
   noRegulation: "レギュレーションがありません。"
  },
 ZeroFiles: {
   drawings: "描画が見つけあせん。",
   photos: "写真が見つけあせん。",
   others: "他が見つけあせん。",
   animations: "アニメーションが見つけあせん。",
   videos: "映画が見つけあせん。",
   files: "ヒレが見つけません。"
  },
 Forgotten: {
   title: "Did you forget your password?",
   subtitle: "Please enter your e-mail.",
   buttonAria: "button to reset the button",
   success: "Check your mailbox."
  },
 ResetPassword: {
   wrongValues: "価値観は同じではありません。",
   success: "パスワードのリセットに成功しました！",
   failed: "パスワードのリセットに失敗しました。もう一度やり直してください。"
  },
 DeletionFile: {
   loadingText: "消してます",
   deletionButton: "消します",
   title: "ファイルが消します",
   question: "Are you sure? You can't undo this action afterwards.",
   cancelButton: "取り消します",
   deleteButton: "消します",
   deleting: "ファイルが消してます",
   deleted: "ファイルが消しました。"
  },
 DeletionPost: {
   title: "掲示が消します",
   deleting: "掲示が消してます。",
   deleted: "掲示が消しました。"
  },
 Contact: {
   title: "初めまして！連絡を入れたいのがうれしいです。",
   subTitleFirst: "もし、オピニオンか誤りが呉れたいです、このおかげで、フォーマットが送りますくだし。このサービスは加筆私の手伝ってます。",
   toFAQ: "そちら様も",
   toFAQHere: "よくあるご質問",
   dot: "見えます。",
   chooseTitle: "題が選ぶ",
   suggestion: "暗示",
   problem: "問題",
   success: "メールが送りました。メッセージ有り難うございました。 出来るのは限り早期,この問題はこの発案が考えますか私が扱います。",
   fail: "メールが送りませんでした。 インターネット接続が見えてください。",
   titleInput: "題",
   message: "メッセージ",
   ariaSend: "メールが送るのボタン"
  },
 FAQ: {
   head1: "このサービスが無料ですか。",
   body1: "はい、無料です。然しながら、もし、追加的の余沢が投じりたいだったら、PREMIUMかGOLDの計は購入です。",
   head2: "PREMIUMかGOLDの計が何余沢上げりますか。",
   body2: "PREMIUMとGOLDの計が取引先の支援（先決 ー GOLD）とより小さいファイルサイズの制限のは設ける。もっと計を、",
   body2Link: "ヘレ",
   body2dot: "、知れます。",
   head3: "PREMIUM/GOLDの計が使わなければなりませんか。",
   body3: "いいえ、使わなければなりませんでした。FREEの計が使えます。",
   head4: "アカウントがどのように消せますか。",
   body4: "「アカウント」サブページの最初のブックマークにある「消します」ボタンをクリックするのと選択を確定するだけです。確認後、すべてのデータの削除が始まります。"
  },
 DeletionAccount: {
   button: "消します",
   title: "アカウントが消します",
   body: "違いないですか。この作用が取り消せません。",
   cancel: "取り消します",
   deletionAccount: "アカウントが消してます。"
  },
 Plans: {
   title: "Pfartistsのプラン",
   subTitle: "最適なプランをお選びください",
   period: "/月例",
   grLength: "無制限のグラフィックスのファイル＊",
   animLength: "無制限のアニメーション＊",
   vidLength: "無制限の映画＊",
   grAnimSize: "グラフィックファイルとアニメーションは1MBまで。",
   grAnimSizeP: "グラフィックファイルとアニメーションは3MBまで。",
   grAnimSizeG: "グラフィックファイルとアニメーションはMBまで。",
   vidSize: "動画サイズ15MBまで",
   vidSizeP: "動画サイズ50MBまで",
   vidSizeG: "動画サイズ200MBまで",
   noAds: "広告がありません",
   support: "補助の取引先＊＊",
   pSupport: "先決の補助の取引先＊＊＊",
   choosePlan: "プランが選ぶ",
   formats: "＊対応フォーマット： ",
   supInfo: "＊＊四営業日以内にメールにてご連絡ください。",
   pSupInfo: "＊＊＊二営業日以内にメールにてご連絡ください。さらにSlackにアクセス。"
  },
 EmailVerification: {
   sendedSuccess: "メールをご確認の上、その中にあるリンクをクリックしてください。",
   expired: "認証コードの有効期限が切れているか、無効になっています。ボタンをクリックして、新しい確認用Eメールリンクを再度送信してください。",
   verified: "メールを確認しました。"
  },
 Settings: {
   title: "設定",
   appearance: "外観",
   dark_mode: "暗いのモード"
  },
 Date: {
   second: "秒",
   minute: "分",
   hour: "時間",
   day: "日",
   yearDateSeparator: "年"
  },
 unknownError: "おっと？！何かが間違っていた。",
 error: "遣り返すかインターネット接続が見えてください。",
 edit: "エディト",
 cancel: "取り止める",
 chooseTag: "タグが選ぶ"
} as const;