...
import { getDoc, getDocs, limit, query, QueryDocumentSnapshot, startAfter, where } from 'firebase/firestore';
...
import { adminInGroups, user, groups, moderatorsGroups, membersGroup } from 'references/referencesFirebase';
...
export const GroupUser = ({ uidUser }: GroupUserType) => {
  ...
  useEffect(() => { !!uidUser && firstAdminList() }, []);
  ...
  useEffect(() => { !!uidUser && firstModeratorsList() }, []);
  ...
  useEffect(() => { !!uidUser && firstMembersList() }, [])
