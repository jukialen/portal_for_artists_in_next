...
export const SkeletonRootLayout = ({ children, isUser, userMenuComponents }: ChildrenType) => {
  ...
  useEffect(() => {
    ...
    const isIn = (list: string[]) => list.some((p) => normalize(path) === p || normalize(path).startsWith(p + '/'));
      ...
      <div className={styles.container}>
        ...
        <main
          ...
          <section className={styles.workspace}>
            {checkUserAndPath ? <p className={styles.loading}>Loading...</p> : children}
          </section>
        </main>
      </div>
    </div>
