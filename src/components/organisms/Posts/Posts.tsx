...
export const Posts = ({ groupId, locale, userId, pseudonym, name, firstPosts }: GroupsPropsType) => {
    ...
    <section className={styles.posts}>
      {!!postsArray && postsArray?.length > 0 ? (
        postsArray?.map((post: PostsType, index) => (
          <Post key={index} userId={userId} pseudonym={pseudonym}  name={name} postOnGroup={post!} />
        ))
      ) : (
